...
const StyledProcceedBox = styled.div`
  ...
  > p {
    font: 300 var(--font-size-medium-33);
    line-height: 150%;
    letter-spacing: 0;
    color: var(--light-900);
    text-align: center;
        ...
        <>
          ...
          {currentPoint === "cart" && (
            <>
              <ProgressBar
                showInnerBar={true}
                progress={totalPrice > 100 ? 100 : totalPrice}
                barHeight={6}
                outerBarColor={"var(--light-600)"}
                innerBarColor={"var(--green-200)"}
              />
              <OfferBox>
                <OfferText>
                  Get Free <span>Shipping</span> for orders over{" "}
                  <span className="price">{formatCurrency(100)}</span>{" "}
            ...
            <>
              <Filter className="check-boxes">
                <Filter.Check
                  ...
                  handleChange={() =>
                    setChecked((checked) => [!checked[0], checked[1]])
                  }
                  checked={checked[0]}
                />
                <Filter.Check
