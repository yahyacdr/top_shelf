...
import { getOrderStatus, getTotalItems } from "../../features/cart/cartSlice";
...
const ProgressBox = memo(() => {
  const { currentPoint, setCurrentPoint, progress, setProgress } =
    useProgress();
  const checkoutProgressBarRef = useRef(null);
  const orderProgressBarRef = useRef(null);
  const isOrderPlaced = useSelector(getOrderStatus);
  ...
  useEffect(() => {
    ...
    if (orderProgressBarRef && checkoutProgressBarRef) {
      if (currentPoint === "order") {
        orderProgressBarRef.current.classList.add("advance");
      }
      if (currentPoint === "checkout") {
        checkoutProgressBarRef.current.classList.add("advance");
      }
      if (currentPoint === "cart") {
        checkoutProgressBarRef.current.classList.remove("advance");
        orderProgressBarRef.current.classList.remove("advance");
      }
    }
  }, [currentPoint, checkoutProgressBarRef, orderProgressBarRef]);
    ...
    <StyledProgressBox>
      <Box>
        ...
        >
          {currentPoint === "cart" && (
            <>
              <ImgContainer className={currentPoint === "cart" && "active"}>
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 14 14"
                ...
                >
                  <path
                    ...
                    stroke={currentPoint === "cart" ? "white" : "#05422C"}
                    strokeMiterlimit="10"
                    strokeLinecap="round"
          ...
          {(currentPoint === "checkout" || currentPoint === "order") &&
            !!itemCount && <CheckAnimation />}
          ...
          <Card.TitleItem
            ...
            className={(currentPoint === "cart" ? "show " : "") + "🛒"}
          >
            shopping cart
          </Card.TitleItem>
        </Point>
        ...
        <ProgressBar
          ...
          ref={checkoutProgressBarRef}
        />

        <Point
          onClick={() => {
            setProgress(() => {
              return { order: 0, checkout: 100 };
            });
        ...
        >
          {currentPoint !== "order" && (
            <>
              <ImgContainer className={currentPoint === "checkout" && "active"}>
                <svg
                  width="15"
                  height="14"
                  viewBox="0 0 15 14"
                ...
                >
                  <path
                    ...
                    stroke={currentPoint === "checkout" ? "white" : "#05422C"}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
          ...
          {currentPoint === "order" && !!itemCount && <CheckAnimation />}

          {currentPoint === "order" && !itemCount && (
            <>
              <ImgContainer>
                <svg
                  width="15"
          ...
          <Card.TitleItem
            ...
            className={(currentPoint === "checkout" ? "show " : "") + "✅"}
          >
            checkout
          </Card.TitleItem>
        </Point>
        ...
        <ProgressBar
          showInnerBar={currentPoint === "order"}
          progress={progress.order}
          outerBarColor={"#c3d2cc"}
          innerBarColor={"var(--green-900)"}
          ref={orderProgressBarRef}
        />

        <Point
          onClick={() => {
            setProgress(() => {
              return { checkout: 100, order: 100 };
            });
        ...
        >
          {!isOrderPlaced && (
            <>
              <ImgContainer className={currentPoint === "order" && "active"}>
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 14 14"
                ...
                >
                  <path
                    ...
                    stroke={currentPoint === "order" ? "white" : "#05422C"}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
          ...
          <Card.TitleItem
            ...
            className={(currentPoint === "order" ? "show " : "") + "📦"}
          >
            order complete
          </Card.TitleItem>
        </Point>
